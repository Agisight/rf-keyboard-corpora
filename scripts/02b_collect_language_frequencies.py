# -*- coding: utf-8 -*-
# 02b_collect_language_frequencies.py → metadata/frequencies_by_language.csv
...
DATA_DIR = Path("data")
OUT_CSV  = "metadata/frequencies_by_language.csv"
VERBOSE  = True
...
def _first_vendor_freq_path(lang: str) -> Optional[str]:
    # поддерживаем и 'frequencies', и часто встречающееся 'frequences'
    patterns = [
    ...
    # если задан предпочтительный вендор — фильтруем
...
def _vendor_from_path(p: str) -> str:
    parts = Path(p).parts
    # ожидаем data/<lang>/<vendor>/frequen(cies|ces)/...
    if "data" in parts:
        i = parts.index("data")
        if i + 2 < len(parts):
            return parts[i + 2]
...
def canonicalize_variant(s: str) -> str:
    ...
    - если строка содержит ᵸ / ᴴ / ʰ в любом месте → вернуть ровно 'ᵸ'
      (объединяем А̄ᵸ, Аᴴ, Аᵸ и т.п. в единый вариант 'ᵸ')
    - иначе: NFC + UPPERCASE.
    """
...
def main():
    ...
    for lang in langs:
        ...
        # суммируем по канонизированному варианту
        Csum: Dict[str, float] = {}
        M_seen: float = 0.0
        ...
        for variant, Ci in sorted(Csum.items()):
            ...
            rows_out.append({
                "lang_code": lang,
                "vendor": vendor,
                "variant": variant,  # уже канонизировано (NFC+UPPERCASE или 'ᵸ')
